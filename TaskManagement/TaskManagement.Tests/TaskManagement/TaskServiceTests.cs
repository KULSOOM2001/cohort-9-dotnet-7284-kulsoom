...
namespace TaskManagement.Tests.TaskManagement
{
    public class TaskServiceTests
    {
        ...
        [Fact]
        public async Task DeleteTaskAsync_AsRegularUser_CannotDeleteAnotherUsersTask()
        {
            var context = await SeedUsersAsync(GetInMemoryDbContext());
            var repository = new TaskRepository(context);
            var userRepository = new UserRepository(context);
            ...
            var success = await service.DeleteTaskAsync(created.Id, currentUserId: 1, currentUserRole: "User");

            Assert.False(success);
        }

        [Fact]
        public async Task DeleteTaskAsync_AsRegularUser_CanDeleteOwnTask()
        {
            var context = await SeedUsersAsync(GetInMemoryDbContext());
            var repository = new TaskRepository(context);
            var userRepository = new UserRepository(context);
            ...
            var success = await service.DeleteTaskAsync(created.Id, currentUserId: 2, currentUserRole: "User");
        ...
        public async Task DeleteTaskAsync_AsAdmin_Succeeds()
        {
            ...
            var success = await service.DeleteTaskAsync(created.Id, currentUserId: 1, currentUserRole: "Admin");
